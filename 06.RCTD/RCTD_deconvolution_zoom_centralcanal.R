...
for (patient in patients) {
  for (mode in modes) {
    ...
    for (im in ims) {
      ...
      ################################
      imagerowmin <- min(imagerow[objectST@images[[1]]@coordinates$row > 18 & 
                                  objectST@images[[1]]@coordinates$row < 41])
      imagerowmax <- max(imagerow[objectST@images[[1]]@coordinates$row > 18 & 
                                  objectST@images[[1]]@coordinates$row < 41])
      imagecolmin <- min(imagecol[objectST@images[[1]]@coordinates$col > 53 & 
                                  objectST@images[[1]]@coordinates$col < 97])
      imagecolmax <- max(imagecol[objectST@images[[1]]@coordinates$col > 56 & 
                                  objectST@images[[1]]@coordinates$col < 97])
